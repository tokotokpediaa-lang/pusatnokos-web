...
const API_ADMIN = {
  ...
  FORCE_CANCEL_ORDER:   '/api/admin/force-cancel-order',
} as const;
...
export default memo(function AdminPanelPage({ showToast, isAdmin }) {
  const [activeTab, setActiveTab] = useState<'overview'|'users'|'deposits'|'orders'|'settings'|'info'>('overview');
  ...
  const forceCancelingIds = useRef<Set<string>>(new Set());
  const [otpWarning, setOtpWarning] = useState<{number: string; otp: string} | null>(null);
  ...
  const handleForceCancel = async (order: any) => {
    if (!isAdmin) return;
    const key = `${order.userId}_${order.id}`;
    if (forceCancelingIds.current.has(key)) return showToast('Sedang diproses.', 'error');
    forceCancelingIds.current.add(key);
    setIsProcessing(true);
    try {
      await secureApiCall(API_ADMIN.FORCE_CANCEL_ORDER, { orderId: order.id, userId: order.userId });
      showToast('Order berhasil di-force cancel. Saldo dikembalikan.', 'success');
    } catch (e: any) {
      // Kalau order sudah punya OTP → tampilkan warning dengan nomor & OTP
      if (e?.hasOtp || e?.status === 409) {
        setOtpWarning({ number: e?.number || '—', otp: e?.otp || '—' });
      } else {
        showToast(e.message || 'Gagal force cancel.', 'error');
      }
    } finally {
      forceCancelingIds.current.delete(key);
  ...
  const tabs = [
    ...
    { id:'info',      label:'Info & Konten', icon: Megaphone },
  ];
  ...
  return (
    <div className="max-w-[90rem] mx-auto space-y-6">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {isActive && (
                            <div onClick={e => e.stopPropagation()}>
                              <ForceCancelButton
                                order={{ ...o, orderId: o.id, userId: o.userId }}
                                adminToken={''}
                                getToken={getToken}
                                onSuccess={() => {}}
                                showToast={showToast}
      ...
      {/* ═══ TAB: INFO & KONTEN ═══ */}
      {activeTab==='info' && (
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5 md:p-7">
      ...
      {otpWarning && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center px-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#0f0a02] border border-orange-500/40 p-7 rounded-2xl w-full max-w-sm shadow-[0_0_40px_rgba(251,146,60,0.2)] relative overflow-hidden">
            <div className="absolute -top-16 -right-16 w-40 h-40 bg-orange-600/10 blur-3xl rounded-full"/>
            <div className="relative z-10 text-center">
              <div className="w-14 h-14 bg-orange-500/10 text-orange-400 flex items-center justify-center rounded-2xl mx-auto mb-4 border border-orange-500/20">
                <AlertTriangle className="w-7 h-7"/>
              </div>
              <h3 className="text-lg font-black text-white uppercase mb-1">OTP Sudah Masuk!</h3>
              <p className="text-gray-400 text-xs mb-5 leading-relaxed">Order ini sudah menerima OTP. Force cancel dibatalkan untuk menghindari kerugian.</p>

              <div className="space-y-3 text-left mb-6">
                <div className="bg-white/[0.04] border border-white/[0.08] rounded-xl p-3">
                  <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-1">Nomor</p>
                  <p className="text-white font-mono font-bold text-sm">{otpWarning.number}</p>
                </div>
                <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-3">
                  <p className="text-[10px] text-orange-400 font-black uppercase tracking-widest mb-1">Kode OTP</p>
                  <p className="text-orange-300 font-mono font-black text-2xl tracking-widest text-center">{otpWarning.otp}</p>
                </div>
              </div>

              <button
                className="w-full py-2.5 text-sm font-bold bg-orange-600 hover:bg-orange-500 text-white rounded-xl transition-all"
                onClick={() => setOtpWarning(null)}>
                MENGERTI
              </button>
            </div>
          </div>
