...
function useCountdown(
  ...
  const [seconds, setSeconds] = React.useState(calcSeconds);
  const onExpireRef = React.useRef(onExpire);
  React.useEffect(() => { onExpireRef.current = onExpire; }, [onExpire]);

  React.useEffect(() => {
    if (!isActive || !expiresAt) { setSeconds(0); return; }
    const tick = () => {
...
// ── Service metadata ──────────────────────────────────────────────────────────
const SERVICE_META: Record<string, { name: string; color: string; bg: string }> = {
  tg:  { name: 'Telegram',  color: '#2AABEE', bg: 'bg-sky-500/20' },
  wa:  { name: 'WhatsApp',  color: '#25D366', bg: 'bg-green-500/20' },
  ig:  { name: 'Instagram', color: '#E1306C', bg: 'bg-pink-500/20' },
  go:  { name: 'Google',    color: '#4285F4', bg: 'bg-blue-500/20' },
  tw:  { name: 'Twitter',   color: '#1DA1F2', bg: 'bg-sky-400/20' },
  fb:  { name: 'Facebook',  color: '#1877F2', bg: 'bg-blue-600/20' },
  tt:  { name: 'TikTok',    color: '#fe2c55', bg: 'bg-red-500/20' },
  li:  { name: 'LINE',      color: '#06C755', bg: 'bg-green-600/20' },
  vk:  { name: 'VK',        color: '#4C75A3', bg: 'bg-blue-400/20' },
  vi:  { name: 'Viber',     color: '#7360F2', bg: 'bg-purple-500/20' },
};

function getServiceMeta(serviceId: string) {
  if (!serviceId) return undefined;
  return SERVICE_META[serviceId.toLowerCase()] ?? { name: serviceId.toUpperCase(), color: '#6366f1', bg: 'bg-indigo-500/20' };
}

function getSAServiceMeta(serviceId: string, saName?: string) {
  return getServiceMeta(serviceId);
}

function ServiceIcon({ service, className = '' }: { service: any; className?: string }) {
  const initial = service?.name?.[0] ?? '?';
  return (
    <div
      className={`flex items-center justify-center rounded-xl font-black text-white ${service?.bg ?? 'bg-white/10'} ${className}`}
