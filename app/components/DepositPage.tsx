...
import {
  ...
  query, where, orderBy, limit, addDoc, updateDoc, increment, runTransaction
...
export default memo(function DepositPage({ user, showToast }) {
  ...
  const savedQr = typeof window !== 'undefined' ? (() => { try { return JSON.parse(sessionStorage.getItem('pn_qr_state') || 'null'); } catch { return null; } })() : null;
  ...
  const presets = [5000, 25000, 50000, 100000, 500000];
            ...
            <>
              <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-8 duration-300 relative z-10">
                <div className="text-center mb-8">
                  ...
                  <p className="text-gray-400 mt-2 text-sm md:text-base">{PAYMENT_DETAILS[method].info || 'Silakan transfer sesuai dengan instruksi di bawah ini'}</p>
                </div>
