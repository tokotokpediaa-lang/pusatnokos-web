...
const InfoPage       = dynamic(() => import('@/app/components/InfoPage'),       { ssr: false });
...
import { 
  ...
  Download, Ban, RotateCcw, Megaphone, Info
} from 'lucide-react';
...
export default function App() {
  const [currentPage, setCurrentPage] = useState(() => { 
    try { 
      const saved = sessionStorage.getItem('pn_current_page') || 'landing';
      // Guard: pin_verify/pin_setup sudah dihapus, redirect ke landing
      if (['pin_verify', 'pin_setup'].includes(saved)) return 'landing'; // ✅ FIX: PIN pages redirect to landing
      return saved;
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        ...
        // ✅ FIX: tambahkan 'pin_verify' & 'pin_setup' agar tidak stuck di halaman PIN
        const PIN_PAGES = ['landing', 'login', 'register', 'pin_verify', 'pin_setup'];
        try {
          setCurrentPage(curr => PIN_PAGES.includes(curr) ? 'dash_home' : curr);
        } catch {
          setCurrentPage(curr => PIN_PAGES.includes(curr) ? 'dash_home' : curr);
        } finally {
          setAuthLoading(false);
        }
        // ✅ Tampilkan panduan sekali per sesi login
        try {
          if (!sessionStorage.getItem('pn_guide_seen')) {
            setShowGuide(true);
            sessionStorage.setItem('pn_guide_seen', '1');
          }
        } catch {}
      } else {
        setUser(null);
  ...
  const navigate = (page) => {
    ...
    setCurrentPage(page);
    // ✅ FIX: simpan SEMUA halaman valid ke sessionStorage, bukan hanya 'dash_'
    // Sebelumnya 'register' & 'login' tidak disimpan, sehingga state hilang saat re-render
    try { 
      const publicPages = ['landing', 'login', 'register'];
      if (page.startsWith('dash_') || publicPages.includes(page)) {
        sessionStorage.setItem('pn_current_page', page);
      }
    } catch {}
  };
  ...
  return (
    ...
    <div className={`min-h-screen ${THEME.bg} text-gray-300 selection:bg-red-500/30 selection:text-white overflow-x-hidden flex flex-col`} style={{fontFamily:"'Sora',sans-serif"}}>
      ...
      <div className="flex-1 w-full h-full relative">
        ...
        ) : user && (currentPage === 'pin_verify' || currentPage === 'pin_setup') ? (
          // ✅ FIX: PIN pages tidak boleh tampil — redirect paksa ke dashboard
          (() => { setTimeout(() => navigate('dash_home'), 0); return null; })()
        ) : (
          <DashboardLayout 
      ...
      {showGuide && <WelcomeGuideModal onClose={() => setShowGuide(false)} />}
    </div>
    </AppContext.Provider>
  );
}
  ...
  return (
    <div className="relative bg-[#060001]">
      ...
      <div className="pt-16 sm:pt-24 min-h-screen">
        ...
        {(currentPage === 'login' || currentPage === 'register') && <AuthPage key={currentPage} type={currentPage} navigate={navigate} showToast={showToast} />}
      </div>
  ...
  return (
    <div className="bg-[#000000] overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex flex-col justify-center pt-28 pb-16 overflow-hidden">
        {/* Background: subtle dot grid — pointer-events-none agar tidak block klik nav */}
        ...
        <div className="absolute top-[-100px] right-[-100px] w-[600px] h-[600px] bg-red-700/20 blur-[150px] rounded-full pointer-events-none" />
        <div className="absolute bottom-0 left-[-100px] w-[400px] h-[400px] bg-red-900/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      {/* ═══ CTA ═══════════════════════════════════════════════════════ */}
      <section className="py-20 md:py-32 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-red-950/40 via-black to-black pointer-events-none" />
        <div className="absolute inset-0 pointer-events-none" style={{backgroundImage:'radial-gradient(rgba(220,38,38,0.06) 1px,transparent 1px)',backgroundSize:'32px 32px'}} />
...
function DashboardLayout({ user, balance, currentPage, navigate, logout, orders, isLoadingOrders, transactions, isLoadingTransactions, showToast, inventory, maintenance = null }) {
  ...
  const MENUS = [
    ...
    { id: 'dash_info', label: 'Info', icon: Info },
    ...
    <div className="flex h-[100dvh] overflow-hidden bg-[#040101]"> 
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative pb-[env(safe-area-inset-bottom,0px)]">
        ...
        >
          ...
          {currentPage === 'dash_info' && <InfoPage />}
  ...
  const steps = [
    {
      ...
      desc: 'Buat akun dalam 30 detik menggunakan email aktif kamu.',
      detail: [
        'Klik tombol Daftar Gratis di halaman utama',
        'Isi nama, email, dan password minimal 8 karakter',
        'Cek email kamu — akan ada kode OTP 6 digit',
        'Masukkan kode OTP, lalu login dengan akun baru kamu',
      ],
    },
    {
      num: '2',
      title: 'Isi saldo',
      desc: 'Top up saldo terlebih dahulu sebelum membeli nomor.',
      detail: [
        'Masuk ke menu Isi Saldo di sidebar',
        'Pilih nominal yang diinginkan (minimal Rp 5.000)',
    ...
    {
      num: '3',
      title: 'Pilih layanan & beli nomor',
      desc: 'Cari aplikasi yang kamu butuhkan, lalu beli nomor instan.',
      detail: [
        'Buka menu Beli Nomor dan cari layanan (misal: WhatsApp, Telegram)',
        'Pilih Server 1 atau Server 2 — keduanya menyediakan stok berbeda',
        'Klik ikon keranjang, konfirmasi harga, lalu klik Beli Sekarang',
        'Nomor virtual akan langsung muncul di Riwayat Pesanan',
      ],
    },
    {
      num: '4',
      title: 'Gunakan nomor & terima OTP',
      desc: 'Masukkan nomor ke aplikasi tujuan dan tunggu kode OTP masuk.',
      detail: [
        'Salin nomor yang didapat dari Riwayat Pesanan',
        'Masukkan nomor ke aplikasi (WhatsApp, Telegram, dll) saat diminta verifikasi',
        'Kode OTP akan muncul otomatis di halaman riwayat pesanan kamu',
        'Jika OTP tidak masuk dalam 5 menit, batalkan pesanan — saldo dikembalikan',
      ],
    },
  ];

  const faqs = [
    { q: 'Apa bedanya Server 1 dan Server 2?', a: 'Server 1 dan Server 2 menggunakan provider berbeda dengan stok yang berbeda. Jika Server 1 habis, coba Server 2.' },
    { q: 'OTP tidak masuk, apa yang harus dilakukan?', a: 'Tunggu hingga 5 menit. Jika belum masuk, batalkan pesanan di Riwayat Pesanan — saldo otomatis dikembalikan.' },
    { q: 'Apakah nomor bisa dipakai lebih dari sekali?', a: 'Tidak. Setiap nomor hanya untuk satu kali verifikasi OTP. Untuk verifikasi berikutnya, beli nomor baru.' },
    { q: 'Berapa lama saldo berlaku?', a: 'Saldo tidak memiliki batas waktu dan akan tetap ada di akun kamu.' },
  ];

  const tips = [
    { label: 'Harga mulai dari', val: 'Rp 1.900 / nomor' },
    { label: 'OTP masuk dalam', val: '< 1 menit' },
    { label: 'Layanan tersedia', val: '90+ aplikasi' },
    { label: 'Jika gagal', val: 'Saldo dikembalikan' },
  ];

  return createPortal(
    <div
      className="fixed inset-0 z-[999] flex items-center justify-center px-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200"
      onClick={onClose}
    >
      <div
        className="bg-[#0a0101] border border-red-900/40 rounded-2xl w-full max-w-lg shadow-[0_0_60px_rgba(220,38,38,0.15)] flex flex-col animate-in slide-in-from-bottom-4 duration-250"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-white/[0.06] shrink-0">
          <div>
            <p className="text-[10px] text-red-400/60 uppercase tracking-widest font-bold mb-0.5">Selamat Datang</p>
            <h3 className="text-lg font-black text-white">Panduan Pengguna Baru</h3>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 bg-white/5 hover:bg-white/10 rounded-xl flex items-center justify-center transition-colors border border-white/[0.06]"
          >
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="space-y-2">
            {steps.map((step, i) => (
              <div
                ...
                className={`rounded-xl border cursor-pointer transition-all duration-150 ${activeStep === i ? 'border-red-500/50 bg-red-500/[0.06]' : 'border-white/[0.07] bg-white/[0.02] hover:border-white/20'}`}
              >
                <div className="flex items-center gap-3 px-4 py-3">
                  <div className="w-7 h-7 rounded-full bg-red-600 text-white text-xs font-black flex items-center justify-center shrink-0">{step.num}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white">{step.title}</p>
                    <p className="text-[11px] text-gray-500 mt-0.5">{step.desc}</p>
                  </div>
                  <ChevronDown className={`w-4 h-4 text-gray-500 shrink-0 transition-transform ${activeStep === i ? 'rotate-180 text-red-400' : ''}`} />
                </div>
                {activeStep === i && (
                  <div className="px-4 pb-4 pt-1">
                    <ul className="space-y-1.5 pl-2 border-l-2 border-red-600/30">
                      {step.detail.map((d, j) => (
                        <li key={j} className="text-xs text-gray-400 pl-3">{d}</li>
                      ))}
                    </ul>
                  </div>
                )}
          ...
          <div className="grid grid-cols-2 gap-2">
            {tips.map((t, i) => (
              <div key={i} className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-3 py-2.5">
                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-0.5">{t.label}</p>
                <p className="text-sm font-black text-white">{t.val}</p>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-1.5">
            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-2">Pertanyaan Umum</p>
            {faqs.map((faq, i) => (
              <div key={i} className="bg-white/[0.02] border border-white/[0.06] rounded-xl overflow-hidden">
                ...
                >
                  <span className="text-sm text-white font-bold pr-3">{faq.q}</span>
                  <ChevronDown className={`w-4 h-4 text-gray-500 shrink-0 transition-transform ${openFaq === i ? 'rotate-180' : ''}`} />
                </button>
                {openFaq === i && (
                  <p className="text-xs text-gray-400 px-4 pb-3 border-t border-white/[0.05] pt-2">{faq.a}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="px-6 py-4 border-t border-white/[0.06] shrink-0">
          <button
            onClick={onClose}
            className="w-full py-3.5 bg-red-600 hover:bg-red-500 active:scale-[0.98] text-white font-black text-sm rounded-xl transition-all shadow-[0_4px_20px_rgba(220,38,38,0.35)] uppercase tracking-widest"
          >
            Mulai Gunakan Pusat Nokos
          </button>
        </div>
      </div>
    </div>,
