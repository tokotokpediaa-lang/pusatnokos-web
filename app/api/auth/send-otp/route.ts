...
// MIGRATION: nodemailer → Resend
// Semua logic (rate limit, hash OTP, Turnstile, JWT) tetap sama.
// Hanya bagian pengiriman email yang diganti ke Resend.
...
export async function POST(req: Request) {
  try {
    ...
    // ✅ Verifikasi JWT jika Authorization header disertakan (flow reset PIN)
    ...
    const { error: sendError } = await resend.emails.send({
      from:    `PusatNokos <noreply@pusatnokos.my.id>`,
      to:      normalizedEmail,
