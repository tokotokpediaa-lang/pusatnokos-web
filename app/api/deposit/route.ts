...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 3 & 4. Rate limit atomik via Firestore transaction ───────────────────
    // FIX: check + set dalam satu transaction agar tidak ada race condition
    const rateLimitRef = adminDb.collection('rateLimits').doc(`deposit_${userId}`);
    try {
      await adminDb.runTransaction(async (t) => {
        const snap = await t.get(rateLimitRef);
        if (snap.exists) {
          const elapsed = Date.now() - (snap.data()?.timestamp ?? 0);
          if (elapsed < COOLDOWN_MS) {
            const remaining = Math.ceil((COOLDOWN_MS - elapsed) / 1000);
            throw new Error(`COOLDOWN:${remaining}`);
          }
        }
        t.set(rateLimitRef, { timestamp: Date.now() });
      });
    } catch (err: any) {
